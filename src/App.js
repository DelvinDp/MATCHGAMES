...
class App extends Component {
  ...
  getFilteredimages = () => {
    const {activeTabId,images} = this.state
  ...
  render() {
    const {activeTabId, topImage, clickedImage} = this.state
    const filteredimages = this.getFilteredimages()
    return (
      <div className="app-container">
        <div>{topImage && <img src={topImage.imageUrl} alt="Top side" />}</div>
        ...
        <ul className="imageContainer">
          {filteredimages.map(imageDetails => (
            <ImageItem
              ...
              style={{
                opacity: clickedImage && clickedImage.id === imageDetails.id ? 0.5 : 1,
              }}
            />
          ))}
        </ul>
